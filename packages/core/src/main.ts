...
import { getDebridService, isUsenetDebridService } from './debrid/index.js';
  ...
  private async _preloadNzbs(
    ...
    for (const stream of streamsToPreload) {
      try {
        ...
        if (isUsenetDebridService(debridService) && debridService.preloadNzb) {
          await debridService.preloadNzb(stream.nzbUrl!, category, folderName);
          ...
          logger.debug(`Service ${serviceId} does not support NZB preloading`);
        }
      } catch (error) {
        logger.warn(`Failed to preload NZB`, {
          error: error instanceof Error ? error.message : String(error),
          nzbUrl: stream.nzbUrl ? maskSensitiveInfo(stream.nzbUrl) : undefined,
        });
      }
    }

    logger.info(`NZB preloading complete for ${context.type} ${context.id}`);
  }
}
